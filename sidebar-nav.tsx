...
// Fonction pure, accessible à tous les composants du fichier.
// On lui passe les compteurs live (lowStock / overdueCredits) en paramètres
// plutôt que de dépendre d'un state interne à un composant.
function getBadge(
          ...
          <div className="mt-1 ml-4 space-y-1">
            {item.children.filter((child) => child.href).map((child) => (
              ...
              >
                ...
                {getBadge(child.badgeKey, child.badge, lowStockCount, overdueCount) > 0 && (
                  <span className="bg-danger-500 text-white text-xs px-2 py-0.5 rounded-full">
                    {getBadge(child.badgeKey, child.badge, lowStockCount, overdueCount)}
                  </span>
                )}
              </Link>
            ))}
    ...
    >
      ...
      {!collapsed && (
        <>
          ...
          {getBadge(item.badgeKey, item.badge, lowStockCount, overdueCount) > 0 && (
            <span className="bg-danger-500 text-white text-xs px-2 py-0.5 rounded-full">
              {getBadge(item.badgeKey, item.badge, lowStockCount, overdueCount)}
            </span>
          )}
        </>
      )}
